...
class W4OS3_Avatar
{
	 ...
	 *
	 * @param [int|WP_Post|array] avatar post, or post id, or array with known properties
	 */
	...
	{
		/* DEPRECATED */
		// TODO: convert to use OpenSimulator database directly
	...
	{
		/* handled by W4OS3::add_submenu_page */
	...
	function get_simulator_data()
	{
		...
		if (! empty($condition)) {
			$matches = $w4osdb->get_results(
				"SELECT * FROM UserAccounts LEFT JOIN userprofile ON PrincipalID = userUUID LEFT JOIN GridUser ON PrincipalID = UserID WHERE active = 1 AND $condition"
			);
			if (count($matches) > 1) {
				$message = sprintf(
