<?php
/**
 * Legacy Search library, obsolete
 *
...
$relative_path = str_replace( OSHELPERS_DIR, '', __FILE__ );
error_log( '[DEPRECATED] Helpers ' . $relative_path . ' is deprecated. Please use OpenSim_Search class instead.' .
    ' Called by ' . $file . ':' . $line );
    

$SearchDB = OpenSim_Search::db();
if( ! $SearchDB ) {
  throw new Exception( 'Search database is not available. Please check your configuration.' );
}

return;

// Everything below this point should be converted inside OpenSim_Search class

// if ( ! defined( 'SEARCH_TABLE_EVENTS' ) ) {
// 	define( 'SEARCH_TABLE_EVENTS', 'events' );
// }

# Moved in OpenSim_Search class
...
//   CREATE TABLE IF NOT EXISTS `" . self::$regions_table . '` (
...
// 	}
	...
	// if ( ! $SearchDB->tables_exist( array( self::$regions_table, 'parcels', 'parcelsales', 'allparcels', 'objects', 'popularplaces', self::$events_table, 'classifieds', 'hostsregister' ) ) ) {
