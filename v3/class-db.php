	...
	class W4OS_WPDB extends WPDB {
		...
		public function __construct( $dbuser, $dbpassword = null, $dbname = null, $dbhost = null, $dbport = null ) {
			...
			if ( ! $socket ) {
				...
				error_log( $error );
				// Create and store exception for this credentials set
				$exception = new \Exception( $error, $errno );
				self::$errors[ $credential_key ] = $exception;
			...
			$this->db_connect();
			if ( ! $this->ready ) {
				// Preserve the actual WPDB error information
				error_log( sprintf( __CLASS__ . ' Error connecting to db %s at %s: %s', $this->dbname, $this->dbhost, $this->last_error ) );
				
				// Create exception with original WPDB error details
				$exception = new \Exception( $this->last_error, $this->last_errno ?? 0 );
				self::$errors[ $credential_key ] = $exception;
				$this->setException( $exception );
			}
		}
