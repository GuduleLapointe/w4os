...
W4OS is designed to be easy to set up, but it is still a work in progress. If you encounter any issues, please follow these steps to troubleshoot the problem. Additionally, you may find solutions to similar issues by visiting the [w4os GitHub issues page](https://github.com/GuduleLapointe/w4os/issues/), as other users may have already addressed them.
...
- **Minimum required php version:** The plugin requires a minimum PHP version of 7.3 for the plugin is **7.3**. While using PHP version 8.1 or later is recommended for adhering to general PHP best practices, it won't have a functional impact on the plugin itself.

- Install and enable the following PHP modules:

  - **PHP XMLRPC**: Check if PHP XMLRPC module is installed. If it's not included in the PHP core, you can install it using the appropriate package for your Linux distribution (e.g., php-xmlrpc).
  - **PHP Imagick**: Verify if PHP Imagick module is installed. If it's not available in the PHP core, you can install it using the respective package for your Linux distribution (e.g., php-imagick).

  Note: Although w4os can run without XMLRPC or Imagick, it's strongly discouraged as it will result in missing essential functionalities, such as helpers and profile features.
...
- Enable or disable the "Provide In-world Search" option:
  ...
  - if enabled,
    ...
    - Set the Search register to <http://yourgrid.org/helpers/register.php>, adjusting it with your Site Address but using "http://".

    By using your own search engine, you will limit search results to your grid only, whether Hypergrid is enabled or not.

  - If not enabled and you use a third-party solution like w4os search engine, enter their address in the Search Engine URL field.

    Using an external provider will provide search results from all the grids registered with the same provider. This option is suitable if Hypergrid is enabled on your grid.

  Ensure that you enter the same addresses in the OpenSim.ini file to maintain consistency between the plugin settings and the OpenSimulator configuration.
...
- It is recommended not to enable the Economy feature until all other components are functioning correctly. However, if you have enabled the "Provide Economy Helpers" option, follow these steps:
  ...
  - Accessing this URI should result in a blank page, indicating that the Economy Helpers functionality is set up correctly.

  Enabling the Economy feature allows for the integration of economic systems within your OpenSimulator grid. However, it's important to ensure that all other components are functioning correctly before enabling this option. By following the above steps, you can configure the Economy Base URI and verify its functionality within the w4os plugin.
