...
function w4os_users_filter_avatars_section( $query ) {
	...
	if ( is_admin() && 'users.php' == $pagenow ) {
		if ( isset( $_GET['filter_avatar_top'] ) ) {
			$value = $_GET['filter_avatar_top'];
		} else {
			$value = isset( $_GET['filter_avatar_bottom'] ) ? $_GET['filter_avatar_bottom'] : null;
		}
...
function w4os_process_actions( $args = array() ) {
	...
	if ( $_REQUEST['action'] == 'create_page' && isset( W4OS_PAGES[ $_REQUEST['helper'] ] ) ) {
		...
		if ( ! is_wp_error( $page ) & ! empty( $page ) ) {
			w4os_transient_admin_notice( sprintf_safe( __( 'Page %s already exists.', 'w4os' ), W4OS_PAGES[ $helper ]['name'] ), 'error' );
		} else {
			...
			if ( ! is_wp_error( $page_id ) ) {
				...
				w4os_transient_admin_notice(
					sprintf_safe( __( 'New page %s created.', 'w4os' ), '<a href=' . get_permalink( $page_id ) . '>' . $data['name'] . '</a>' ),
					'success',
				);
			} else {
				w4os_transient_admin_notice( sprintf_safe( __( 'Error while creating page %s.', 'w4os' ), W4OS_PAGES[ $helper ]['name'] ), 'error' );
			}
		}
		wp_redirect( admin_url( 'admin.php?page=' . $_GET['page'] ) );
		exit;
		...
		define(
			...
			array(
				...
				'OfflineMessageURL' => array(
					...
					'recommended'       => helper_url( 'offline.php' ),
					'os_config'         => array(
