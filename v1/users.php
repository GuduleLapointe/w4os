...
function w4os_get_users_ids_and_uuids() {
	...
	$GridAccounts = $w4osdb->get_results(
		"SELECT Email as email, PrincipalID, FirstName, LastName FROM UserAccounts
		WHERE active = 1
	...
	foreach ( $GridAccounts as $key => $row ) {
		...
		$WPGridAccounts = $wpdb->get_results(
			"SELECT user_email as email, ID as user_id, meta_value AS w4os_uuid
			FROM $wpdb->users LEFT JOIN $wpdb->usermeta
			ON ID = user_id AND meta_key = 'w4os_uuid' AND meta_value != '' AND meta_value != '" . W4OS_NULL_KEY . "'",
			OBJECT_K
		);
		...
		if ( isset( $account['PrincipalID'] ) & ! w4os_empty( $account['PrincipalID'] ) ) {
			if ( $account['PrincipalID'] == $account['w4os_uuid'] ) {
				...
				if ( w4os_profile_sync( $account['user_id'], $account['PrincipalID'] ) ) {
					$users_updated[] = sprintf_safe( '<a href=%s>%s %s</a>', get_edit_user_link( $newid ), $account['FirstName'], $account['LastName'] );
				} else {
					$errors[] = '<p class=error>' . sprintf_safe( __( 'Error while updating %1$s %2$s (%3$s) %4$s', 'w4os' ), $account['FirstName'], $account['LastName'], $account['email'], $result ) . '</p>';
				}
			} else {
				...
				if ( is_wp_error( $newid ) ) {
					...
					$users_created[] = sprintf_safe( '<a href=%s>%s %s</a>', get_edit_user_link( $newid ), $account['FirstName'], $account['LastName'] );
				} else {
					$errors[] = '<p class=error>' . sprintf_safe( __( 'Error while updating newly created user %1$s for %2$s %3$s (%4$s) %5$s', 'w4os' ), $newid, $account['FirstName'], $account['LastName'], $account['email'], $result ) . '</p>';
				}
			}
		} elseif ( isset( $account['w4os_uuid'] ) & ! w4os_empty( $account['w4os_uuid'] ) ) {
			w4os_profile_dereference( $account['user_id'] );
			$users_dereferenced[] = sprintf_safe( '<a href=%s>%s</a>', get_edit_user_link( $account['user_id'] ), $account['user_id'] );
			// } else {
...
function w4os_profile_sync( $user_or_id, $uuid = null ) {
	...
	if ( ! w4os_check_db_tables( 'userprofile' ) ) {
		return; // profiles are not enabled on robust
	}
...
function w4os_profile_sync_all() {
	...
	w4os_admin_notice( sprintf_safe( __( '%s local users updated with avatar data', 'w4os' ), count( $updated ) ), 'success' );
	return;
}
