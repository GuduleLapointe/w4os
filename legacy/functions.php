...
function w4os_update_grid_info($rechecknow = false) {
	...
	if(isset($grid_info['message'])) update_option('w4os_offline_helper_uri', $grid_info['message']);
...
function w4os_hop($url = NULL, $string = NULL) {
	if(empty($url)) return;
	if(get_option($url)) $url = get_option('w4os_login_uri');
	$url=preg_replace('#.*://#', '', $url);
	if(empty($string)) $string = $url;
	$class="hop";
