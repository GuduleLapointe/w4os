		...
		if ( isset( $account['PrincipalID'] ) & ! w4os_empty( $account['PrincipalID'] ) ) {
			if ( $account['PrincipalID'] == $account['w4os_uuid'] ) {
				...
				if ( w4os_profile_sync( $account['user_id'], $account['PrincipalID'] ) ) {
					...
					$errors[] = '<p class=error>' . sprintf( __( 'Error while updating %1$s %2$s (%3$s) %4$s', 'w4os' ), $account['FirstName'], $account['LastName'], $account['email'], $result ) . '</p>';
				}
			} else {
				...
				if ( is_wp_error( $newid ) ) {
					...
					$errors[] = '<p class=error>' . sprintf( __( 'Error while updating newly created user %1$s for %2$s %3$s (%4$s) %5$s', 'w4os' ), $newid, $account['FirstName'], $account['LastName'], $account['email'], $result ) . '</p>';
				}
			}
